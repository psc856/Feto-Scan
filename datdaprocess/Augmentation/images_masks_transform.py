...
def array_to_img(x, dim_ordering='default', scale=True):
    ...
    # Get the current image data format (either 'channels_last' or 'channels_first')
    if dim_ordering == 'default':
        dim_ordering = K.image_data_format()

    # If 'channels_first' (Theano), transpose from (C, H, W) to (H, W, C)
    if dim_ordering == 'channels_first':
        x = x.transpose(1, 2, 0)
    
    # Scale pixel values to [0, 255] if 'scale' is True
    if scale:
        x += max(-np.min(x), 0)  # Ensure no negative values
        x /= np.max(x)            # Normalize to [0, 1]
        x *= 255                 # Scale to [0, 255]
...
def img_to_array(img, dim_ordering='default'):
    ...
    # If the image is 3D (height, width, channels)
    if len(x.shape) == 3:
        if dim_ordering == 'channels_first':
            # Convert from (height, width, channels) to (channels, height, width)
            x = x.transpose(2, 0, 1)

    # If the image is 2D (height, width)
    elif len(x.shape) == 2:
        if dim_ordering == 'channels_first':
            # Add a channels dimension (1 channel) -> (1, height, width)
            x = x.reshape((1, x.shape[0], x.shape[1]))
        else:
            # Add a channels dimension (1 channel) -> (height, width, 1)
...
class ImageDataGenerator(object):
    ...
    def __init__(self,
        ...
        # Use K.image_data_format() to get default dim_ordering
        if dim_ordering == 'default':
            dim_ordering = K.image_data_format()
        ...
        # Validate dim_ordering
        if dim_ordering not in {'channels_last', 'channels_first'}:
            raise Exception('dim_ordering should be "channels_last" or "channels_first". '
                             'Received: ', dim_ordering)
        self.dim_ordering = dim_ordering
        
        # Set the index values for rows, columns, and channels based on the image format
        if dim_ordering == 'channels_first':
        ...
        # Handle zoom_range (it should be a scalar or a tuple/list of 2 values)
        if np.isscalar(zoom_range):
            self.zoom_range = [1 - zoom_range, 1 + zoom_range]
        elif len(zoom_range) == 2:
            self.zoom_range = [zoom_range[0], zoom_range[1]]
        else:
            raise Exception('zoom_range should be a float or a tuple/list of two floats. '
                             'Received: ', zoom_range)
...
class NumpyArrayIterator(Iterator):
    def __init__(self, X, y, image_data_generator,
        ...
        # Check if the images and labels have the same length
        if len(X) != len(y):
            raise Exception('X (images tensor) and y (labels) '
                            'should have the same length. '
                            'Found: X.shape = %s, y.shape = %s' % (np.asarray(X).shape, np.asarray(y).shape))

        # Ensure dim_ordering is valid
        if dim_ordering == 'default':
            dim_ordering = K.image_dim_ordering()
        if dim_ordering not in ['th', 'tf','channels_last']:
            raise ValueError(f"Invalid dim_ordering: {dim_ordering}. Must be 'th' or 'tf'.")
        ...
        # Create directory if save_to_dir is provided and does not exist
        if save_to_dir and not os.path.exists(save_to_dir):
            os.makedirs(save_to_dir)
        
        # Initialize the base Iterator class
